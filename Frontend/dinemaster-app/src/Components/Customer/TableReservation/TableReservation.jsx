...
const TableReservation = () => {
    ...
    const handleBookingAction = (targetPath) => {
        if(!date || !time || !selectedTable) { 
            alert("Please complete the reservation details first."); 
            return; 
        }
        addBooking({
                    ...
                    {selectedTable && (
                        <div className="res-actions">
                            ...
                            <div className="btn-stack">
                                <button className="secondary-btn" onClick={() => handleBookingAction('/order-online')}>
                                    Add Food Items
                                </button>
                                <button className="primary-btn" onClick={() => handleBookingAction('/cart')}>
                                    Go to Checkout
                                </button>
                            </div>
                        </div>
                ...
                <div className="floor-plan-card">
                    <div className="floor-grid">
                        {tables.map(table => (
                            <div 
                                key={table.id}
                                className={`table-box ${table.type} ${table.status} ${selectedTable?.id === table.id ? 'active' : ''}`}
                                onClick={() => table.status === "available" && setSelectedTable(table)}
                            >
                                <span className="lbl">{table.label}</span>
                                <span className="cap"><FaUserFriends /> {table.size}</span>
                            </div>
                        ))}
